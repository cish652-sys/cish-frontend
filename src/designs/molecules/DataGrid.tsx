...
// 2. FIX: Replaced 'any' with the specific types from TableRow
const getSafeUrl = (url: string | number | React.ReactNode): string | null => {
      ...
      <div className="w-full">
        {data.length === 0 ? (
          ...
          data.map((row, rowIndex) => {
            const safeUrl = getSafeUrl(row.viewUrl);
                ...
                <div className="w-[60%] flex items-center gap-3">
                  ...
                  {safeUrl ? (
                    <a
                      ...
                      className="text-gray-700 font-medium hover:text-[#599A5E] hover:underline"
                    >
                      {row.title}
                    </a>
                  ) : (
