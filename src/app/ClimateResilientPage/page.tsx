...
function ClimateResilientPage() {
  ...
  const [allClimateData, setAllClimateData] = useState<ApiTechnology[]>([]);
    ...
    const fetchVarieties = async () => {
      try {
        ...
        if (Array.isArray(data) && data.length > 0) {
          ...
          const otherItemsData = climateOnly.slice(3);
  ...
  const handleViewTopItem = (item: TechnologyCardItem) => {
    const fullItem = allClimateData.find((v) => v.id === item.id);
    if (fullItem) {
  ...
  const handleViewOtherItem = (id: number) => {
    const fullItem = allClimateData.find((v) => v.id === id);
    if (fullItem) {
      setSelectedItem(fullItem);
      setIsModalOpen(true);
    }
